...
//Use dbActionCheck in a onBeforeAction to check the requested database action 
//passed as a parameter 'dbAction' (update, insert etc.)
var dbActionCheck = function(){
	var acceptedValues = ["insert", "edit"];
	var dbAct = this.params.dbAction;
	console.log(acceptedValues.indexOf(dbAct));
	if ( acceptedValues.indexOf(dbAct) >= 0 ) {
		this.next();
	}else{ 
...
Router.onBeforeAction(dbActionCheck, {only: 'mfrEdit'});
...
Router.route('/setup/mfrs/:dbAction/:_id', { 	// this route accepts "insert" and "edit" as dbAction
	...
	data: function () {
		...
		if (dbAct==="insert") return {};
		if (dbAct==="edit") return Mfrs.findOne(id);
